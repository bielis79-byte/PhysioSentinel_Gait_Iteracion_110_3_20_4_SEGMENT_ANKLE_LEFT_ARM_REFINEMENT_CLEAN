...
# V110.3.20.4 · MINIMAL DIRECT PIPELINE
...
def _segment_relative_lower_seed_112(torch,model,pose,rotvec,trans,scale,target_df,reference_target_df,betas,zero_trans,reference_pose,axis_map,calibration=None):
    """V110.3.20.4 · Retargeting segmentario absoluto respecto al frame semilla.
    ...
    """
    out={'strategy':'V110.3.20.4 segment-relative hip + geometric knee','accepted':False,'chains':{}}
    try:
...
# V110.3.20.4 · FULL-BODY SEGMENT-DRIVEN RETARGETING
...
def _frame1_segment_posture_fit_200(torch,model,pose_init,target_df,betas,zero_trans,rotvec,scale,trans,axis_map,iterations=64):
...
def _smooth_fullbody_q_sequence_204(torch,model,sequence,betas,zero_trans,axis_map,scale):
    """V110.3.20.4 · suavizado conservador segmentario con tobillos observados y brazo I refinado.

    q7/q14 se conservan como señal distal Ankle→BigToe. El brazo izquierdo recibe
    un filtro cero-fase algo más fuerte y limitador robusto de salto para reducir tirones,
    sin imponer braceo contralateral ni copiar el brazo derecho.
    """
    ok_idx=[i for i,f in enumerate(sequence) if f.get('status')=='ok' and f.get('pose') is not None]
    ...
    P=np.asarray([sequence[i]['pose'] for i in ok_idx],dtype=np.float64); raw=P[:,qids].copy(); sm=raw.copy()
    k5=np.asarray([1,2,3,2,1],dtype=np.float64)/9.0
    k7=np.asarray([1,2,3,4,3,2,1],dtype=np.float64)/16.0
    left={36,37,38,39,40,41,42}
    ankle={7,14}
    diag={}
    for j,qi in enumerate(qids):
        x=raw[:,j]
        if qi in left:
            pad=np.pad(x,(3,3),mode='reflect'); f=np.convolve(pad,k7,mode='valid'); a=.78
        else:
            pad=np.pad(x,(2,2),mode='reflect'); f=np.convolve(pad,k5,mode='valid'); a=.58 if qi in ankle else .68
        y=a*f+(1-a)*x
        rr=float(np.ptp(x)); yr=float(np.ptp(y))
        floor=.90 if qi in left else (.94 if qi in ankle else .92)
        if rr>1e-5 and yr>1e-6 and yr<floor*rr:
            mu=float(np.mean(y)); y=mu+(y-mu)*(floor*rr/yr)
        y=y+(x[0]-y[0])
        d=np.diff(y); med=float(np.median(np.abs(d))) if len(d) else 0.0
        hard=np.deg2rad(3.6 if qi in left else (4.0 if qi in ankle else 5.0))
        cap=max(hard,3.0*med)
        for _ in range(2):
            for n in range(1,len(y)):
                dd=y[n]-y[n-1]
                if abs(dd)>cap: y[n]=y[n-1]+np.sign(dd)*cap
            for n in range(len(y)-2,-1,-1):
                dd=y[n]-y[n+1]
                if abs(dd)>cap: y[n]=y[n+1]+np.sign(dd)*cap
            y=y+(x[0]-y[0])
        sm[:,j]=y
        diag[_SKEL_Q_NAMES_130[qi]]={'raw_range_deg':float(np.degrees(rr)),'refined_range_deg':float(np.degrees(np.ptp(y))),
            'raw_max_step_deg':float(np.degrees(np.max(np.abs(np.diff(x))))) if len(x)>1 else 0.0,
            'refined_max_step_deg':float(np.degrees(np.max(np.abs(np.diff(y))))) if len(y)>1 else 0.0,
            'filter_weight':float(a),'cap_deg_per_frame':float(np.degrees(cap))}
    for r,i in enumerate(ok_idx):
        fr=sequence[i]; pp=np.asarray(fr['pose'],dtype=np.float32).copy(); pp[qids]=sm[r].astype(np.float32)
        pt=torch.tensor(pp,dtype=torch.float32).reshape(1,-1); _clamp_direct_pose_130(torch,pt)
        rv=torch.tensor(fr.get('rotvec',[0,0,0]),dtype=torch.float32); tr=torch.tensor(fr.get('trans',[0,0,0]),dtype=torch.float32)
    ...
    return {'applied':True,'method':'segment FIR; ankle ROM>=94%; left-arm 7-point zero-phase + robust step limiter',
            'qids':qids,'per_q':diag,'raw_ranges':np.ptp(raw,axis=0).astype(float).tolist(),'refined_ranges':np.ptp(sm,axis=0).astype(float).tolist()}
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    smooth=_smooth_fullbody_q_sequence_204(torch,model,sequence,betas,zero_trans,axis_map,scale)
    result={'version':'110.3.20.4','retargeting_mode':'segment_base_alignment_ankle_foot_left_arm_refinement','scale':float(scale.cpu()),
            ...
            'fullbody_smoothing_200':_json_safe_134(smooth),
            'segment_motion_audit_200':_json_safe_134(_segment_motion_audit_200(sequence)),
            'segment_refinement_204':_json_safe_134(smooth)}
...
# V110.3.20.4 · SKEL LOWER-LIMB MOTOR ISOLATION TEST
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.18 · reproductor HTML/JS explícito para la malla SKEL, centrado en pelvis.

    V110.3.13 contenía 75 mallas distintas, pero los controles Plotly no avanzaban
    de forma fiable dentro de Streamlit. Aquí requestAnimationFrame actualiza
    directamente x/y/z de Mesh3d y de los joints, sin depender de fig.frames.
    """
    try:
        ...
        J=mesh_seq.get("joints"); J=np.asarray(J,dtype=np.float32) if J is not None else None
        frame_ids=[int(x) for x in np.asarray(mesh_seq.get("frame_ids",np.arange(len(V))+1)).tolist()]
        if V.ndim!=3 or V.shape[0]<1 or F.ndim!=2 or F.shape[1]<3:
            raise RuntimeError("Dimensiones de malla no válidas para el reproductor V110.3.14.")
        ...
        # V110.3.18 · vista centrada en pelvis, EXCLUSIVAMENTE visual.
        # El NPZ científico conserva coordenadas originales intactas.
        Vviz=V.copy(); Jviz=J.copy() if J is not None else None
        pelvis_idx=norm.get(_simple_name('pelvis'))
        if Jviz is not None and pelvis_idx is not None and len(Jviz)==len(Vviz):
            centers=Jviz[:,pelvis_idx,:].copy()
            Vviz=Vviz-centers[:,None,:]
            Jviz=Jviz-centers[:,None,:]
            # V110.3.20.4 · orientación VISUAL fija desde frame 1:
            # X mediolateral, Y progresión, Z vertical anatómica. No altera NPZ científico.
            try:
                thor=norm.get(_simple_name('thorax')); fr=norm.get(_simple_name('femur_r')); fl=norm.get(_simple_name('femur_l'))
                if thor is not None and fr is not None and fl is not None:
                    ez=_safe_unit_112(Jviz[0,thor]-Jviz[0,pelvis_idx])
                    ex=_safe_unit_112(Jviz[0,fl]-Jviz[0,fr])
                    ex=_safe_unit_112(ex-np.dot(ex,ez)*ez)
                    ey=_safe_unit_112(np.cross(ez,ex)); ex=_safe_unit_112(np.cross(ey,ez))
                    B=np.stack([ex,ey,ez],axis=1).astype(np.float32)
                    Vviz=Vviz@B; Jviz=Jviz@B
            except Exception:
                pass
        vv=np.round(Vviz,5).tolist(); ff=F[:,:3].tolist(); jj=np.round(Jviz,5).tolist() if Jviz is not None else []
        mins=np.nanmin(Vviz.reshape(-1,3),axis=0); maxs=np.nanmax(Vviz.reshape(-1,3),axis=0)
        span=np.maximum(maxs-mins,1e-3); pad=np.maximum(0.08*span,0.03)
        # rangos centrados alrededor de cero para una cámara estable durante la marcha
        rad=np.maximum(np.maximum(np.abs(mins),np.abs(maxs))+pad,0.2)
        ranges=[[-float(rad[d]),float(rad[d])] for d in range(3)]
        payload=_json.dumps({"V":vv,"F":ff,"J":jj,"ids":frame_ids,"edges":edges,"ranges":ranges},separators=(",",":"))
        plotly_js=get_plotlyjs()
        html = f"""<!doctype html><html><head><meta charset='utf-8'><style>
        html,body{{margin:0;background:white;font-family:system-ui;color:#1f2937}}#bar{{display:flex;gap:6px;align-items:center;padding:6px 4px;flex-wrap:wrap}}
        button{{padding:6px 10px;border:1px solid #cbd5e1;background:#fff;border-radius:5px;cursor:pointer}}button:hover{{background:#f1f5f9}}
        #counter{{font-weight:700;min-width:100px}}#slider{{flex:1;min-width:280px}}#plot{{width:100%;height:690px}}#recstatus{{font-size:12px;color:#475569}}.dl{{padding:6px 10px;border:1px solid #16a34a;color:#166534;text-decoration:none;border-radius:5px;font-weight:700}}</style>
        <script>{plotly_js}</script></head><body><div id='bar'><button onclick='playMesh(0.5)'>▶ 0.5×</button><button onclick='playMesh(1)'>▶ 1×</button><button onclick='playMesh(2)'>▶ 2×</button><button onclick='pauseMesh()'>⏸ Pausa</button><button onclick='recordMeshVideo()'>⬇ Vídeo SKEL</button><span id='recstatus'></span><span id='counter'>Frame 1/{len(frame_ids)}</span><input id='slider' type='range' min='0' max='{max(0,len(frame_ids)-1)}' value='0' step='1'></div><div id='plot'></div>
        <script>
        const P={payload}; const gd=document.getElementById('plot'); const slider=document.getElementById('slider'); const counter=document.getElementById('counter'); const recstatus=document.getElementById('recstatus');
        let idx=0, playing=false, speed=1, last=0, raf=0, recorder=null, chunks=[], recording=false;
        const fi=P.F.map(x=>x[0]), fj=P.F.map(x=>x[1]), fk=P.F.map(x=>x[2]);
        function xyz(k){{const q=P.V[k];return [q.map(p=>p[0]),q.map(p=>p[1]),q.map(p=>p[2])];}}
        function joints(k){{if(!P.J.length)return [[],[],[]];const q=P.J[k],x=[],y=[],z=[];for(const e of P.edges){{const a=q[e[0]],b=q[e[1]];x.push(a[0],b[0],null);y.push(a[1],b[1],null);z.push(a[2],b[2],null)}}return[x,y,z];}}
        const a=xyz(0),j=joints(0);
        const data=[{{type:'mesh3d',x:a[0],y:a[1],z:a[2],i:fi,j:fj,k:fk,name:'Malla corporal SKEL',opacity:.88,flatshading:false,lighting:{{ambient:.55,diffuse:.75,specular:.15,roughness:.75}}}},{{type:'scatter3d',mode:'lines+markers',x:j[0],y:j[1],z:j[2],name:'Joints SKEL',marker:{{size:3}}}}];
        const layout={{title:'V110.3.20.4 · SKEL ANIMATED ANATOMICAL MESH · anatomical-axis centered playback',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        Plotly.newPlot(gd,data,layout,{{displaylogo:false,scrollZoom:true,responsive:true}});
        function applyFrame(k){{idx=(k+P.V.length)%P.V.length;const a=xyz(idx),j=joints(idx);Plotly.restyle(gd,{{x:[a[0]],y:[a[1]],z:[a[2]]}},[0]);Plotly.restyle(gd,{{x:[j[0]],y:[j[1]],z:[j[2]]}},[1]);slider.value=idx;counter.textContent='Frame '+P.ids[idx]+'/'+P.ids[P.ids.length-1];}}
        function loop(ts){{if(!playing)return;const dt=60/speed;if(ts-last>=dt){{last=ts;applyFrame(idx+1)}}raf=requestAnimationFrame(loop)}}
        function playMesh(s){{speed=s;playing=true;last=0;if(raf)cancelAnimationFrame(raf);raf=requestAnimationFrame(loop)}}
        function pauseMesh(){{playing=false;if(raf)cancelAnimationFrame(raf);raf=0}}
        function recMime(){{for(const m of ['video/mp4;codecs=h264','video/mp4','video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm']){{if(window.MediaRecorder&&MediaRecorder.isTypeSupported(m))return m}}return ''}}
        async function recordMeshVideo(){{
          if(recording){{try{{recorder.stop()}}catch(e){{}}return}}
          const canvas=gd.querySelector('canvas');
          if(!canvas||!canvas.captureStream||!window.MediaRecorder){{recstatus.textContent='Grabación no disponible en este navegador';return}}
          pauseMesh(); applyFrame(0); await new Promise(r=>setTimeout(r,150));
          const mime=recMime(); const stream=canvas.captureStream(24);
          try{{recorder=new MediaRecorder(stream,mime?{{mimeType:mime,videoBitsPerSecond:4500000}}:{{videoBitsPerSecond:4500000}})}}catch(e){{recstatus.textContent='Error al iniciar grabación: '+e;return}}
          chunks=[]; recording=true; recstatus.textContent='Grabando marcha completa…';
          recorder.ondataavailable=e=>{{if(e.data&&e.data.size)chunks.push(e.data)}};
          recorder.onstop=()=>{{recording=false; const type=recorder.mimeType||mime||'video/webm'; const ext=type.includes('mp4')?'mp4':'webm'; const blob=new Blob(chunks,{{type}}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='PhysioSentinel_SKEL_malla_anatomica.'+ext; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(url),5000); recstatus.textContent='Vídeo descargado ('+ext.toUpperCase()+')'}};
          recorder.start(250);
          for(let k=0;k<P.V.length;k++){{applyFrame(k); await new Promise(r=>setTimeout(r,70))}}
          await new Promise(r=>setTimeout(r,120)); try{{recorder.stop()}}catch(e){{}}
        }}
        slider.addEventListener('input',e=>{{pauseMesh();applyFrame(parseInt(e.target.value)||0)}});
        document.addEventListener('visibilitychange',()=>{{if(document.hidden)pauseMesh()}});
        </script></body></html>"""
        components.html(html,height=770,scrolling=False)
        st.caption("V110.3.20.4 · visor centrado en pelvis y orientado a ejes anatómicos sólo para presentación. El NPZ científico conserva las coordenadas originales. Usa ‘⬇ Vídeo SKEL’ para grabar y descargar la marcha anatómica desde el visor.")
    except Exception as exc:
        st.warning(f"El reproductor HTML V110.3.14 no pudo iniciarse: {type(exc).__name__}: {exc}. Se muestra fallback estático.")
        try:
            import plotly.graph_objects as go
            V=np.asarray(mesh_seq["vertices"],dtype=np.float32); F=np.asarray(mesh_seq["faces"],dtype=np.int32)
            fig=go.Figure([go.Mesh3d(x=V[0,:,0],y=V[0,:,1],z=V[0,:,2],i=F[:,0],j=F[:,1],k=F[:,2],opacity=.88)])
            fig.update_layout(height=700,scene=dict(aspectmode="data"),title="SKEL Mesh · fallback frame 1")
            st.plotly_chart(fig,use_container_width=True,config={"displaylogo":False,"scrollZoom":True})
        except Exception:
            pass


# V110.3.13 · CALIBRACIÓN EMPÍRICA DE LOS DOF SKEL
...
def _lower_limb_vector_dls_196(torch,model,pose,rotvec,trans,scale,target_df,rows,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.20.4 · propagación q(t) de miembros inferiores basada en VECTORES DE SEGMENTO.
    ...
    audit={'accepted':False,'strategy':'V110.3.20.4 relative-segment weighted-XYZ DLS','axis_weights':w.astype(float).tolist(),'chains':{}}
    any_commit=False
...
def _lower_limb_delta_tracking_199_fast(torch,model,pose,rotvec,trans,scale,target_df,prev_target_df,betas,zero_trans,prev_pose,axis_map,profile,acquisition_mode):
    """V110.3.20.4 · solver inferior quirúrgico y rápido.
    ...
    try:
        ...
        chains=[('R',[3,4,5,6],('RHip','RKnee','RAnkle'),('femur_r','tibia_r','talus_r')),
                ('L',[10,11,12,13],('LHip','LKnee','LAnkle'),('femur_l','tibia_l','talus_l'))]
        audit={'accepted':False,'strategy':'V110.3.20.4 fast frame-delta DLS','axis_weights':w.tolist(),'chains':{}}
        with torch.no_grad():
            leg_ids=[3,4,5,6,7,10,11,12,13,14]
        ...
        return pose,{'accepted':False,'strategy':'V110.3.20.4 fast frame-delta DLS','error':f'{type(exc).__name__}: {exc}','non_blocking':True}
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    frames=list((motion or {}).get("frames") or [])
    # V110.3.20.4 MINIMAL DIRECT: sin persistencia de secuencia/malla/NPZ.
    if not frames:
    ...
    st.caption("V110.3.20.4 · flujo directo mínimo: sin migración de NPZ y sin recuperación/persistencia de secuencias o mallas previas. Cada ejecución parte de los 75 frames V104/V107 activos y genera q(t) → skin_verts(t) → malla en memoria.")
    ...
    st.markdown("**Modelo SKEL privado · V110.3.20.4 · Lower-Limb q(t) Propagation Fix**")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    st.markdown("### 🧪 V110.3.20.4 · SKEL Lower Limb Motor Test")
                    ...
                    if isinstance(_mmesh,dict) and isinstance(_mmesh.get('vertices'),np.ndarray):
                        ...
                        st.download_button("⬇️ Descargar Motor Test SKEL (.NPZ)",_mesh_sequence_npz_bytes(_mmesh),"V110_3_20_4_SKEL_lower_limb_motor_test.npz","application/octet-stream",use_container_width=True)

                    st.markdown("### V110.3.20.4 · Propagación temporal 1→75")
                        ...
                        try:
                            ...
                            if len(oks_auto)>=max(2,int(0.9*expected)):
                                ...
                                try:
                                    ...
                                    # V110.3.20.4: fin del camino crítico. No se persiste ni recarga nada aquí.
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        # V110.3.20.4 · sólo memoria de sesión. Si q(t) existe y la malla no,
                        ...
                        st.markdown("### V110.3.20.4 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.markdown('### V110.3.20.4 · Validación sincronizada Side-by-Side')
                        ...
                        if frame_gate_ok:
                            st.info("✅ El frame semilla está validado. Pulsa el botón anterior para resolver frames 2→75; después V110.3.20.4 generará automáticamente skin_verts(t) y la malla corporal animada en la misma ejecución.")
                        else:
